...
let by = document.querySelector('#createdBy')

by.innerHTML = 'Aplicación creada por: <span class = name >HarolDev</span> '
...
function displayToDos() {
  ...
  for (let i = 0; i < aux.length; i++) {
    toDoContainer.appendChild(aux[i]) ;
  }
}
...
function addToDo() {
  ...
  let Ntodo = new ToDo(input.value);
  toDoItems.push(Ntodo); 
  toDoItems.innerHTML = ''
  input.value = ''
  displayToDos();
}
...
// Tu código acá:
  ...
  boton.addEventListener('click', this.addToDo)
